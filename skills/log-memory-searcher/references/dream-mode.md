...
- [做梦模式参考文档](#做梦模式参考文档)
  ...
  - [工作流程](#工作流程)
    ...
    - [第四步：游走 / 分析](#第四步游走--分析)
      ...
      - [无提示：纯自由联想](#无提示纯自由联想)
    - [第五步：产出六种输出](#第五步产出六种输出)
      - [输出1：修改原始日志](#输出1修改原始日志)
      - [输出2（条件触发）：更新或创建 AGENTS.md](#输出2条件触发更新或创建-agentsmd)
      - [输出3：生成独立梦境报告](#输出3生成独立梦境报告)
      - [输出4：回写上游报告（条件触发）](#输出4回写上游报告条件触发)
      - [输出5：追加续梦提示（条件触发）](#输出5追加续梦提示条件触发)
      - [输出6：更新统计](#输出6更新统计)
      - [输出检查清单](#输出检查清单)
  - [做梦模式的约束](#做梦模式的约束)
  - [脚本参数速查](#脚本参数速查)
